...
class Libro(db.Model):
    ...
    def obtener_todos(cls):
        libros = cls.query.all()
        return [libro.serializar() for libro in libros] # Serializar permite más adelante convertir en JSON

    @classmethod
    def obtener_por_id(cls, libro_id):
        libro = cls.query.get(libro_id)
        return libro.serializar() if libro else None
